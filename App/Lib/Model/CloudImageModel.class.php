...
class CloudImageModel {
	//上传文件的最大值
	public $manSize = 2048000;//为2Mb大小，可更改
    ...
    public function getConfig(){
        $config = D('Xdata')->get('admin_Config:cloudimage');
        return $config;
    }
 ...
 /*   public function getUrlPrefix(){
        return $this->config['cloud_image_prefix_urls'];
    }*/
    ...
    protected function error($errorNo) {
         switch($errorNo) {
            case 1:
                ...
                $this->_error = '上传文件大小不符，文件不能超过 '.$size;
                break;
            case 2:
                $size = ini_get("upload_max_filesize");
                if( strpos($size,'M')!==false || strpos($size,'m')!==false ) {
    ...
    private function check($file) {
        ...
        if(!$this->checkSize($file['size'])) {
            $this->_error = '上传文件大小不符,文件不能超过 '.byte_format($this->maxSize);
            return false;
        }
