	...
	/*this._loadCss = function(){
		...
		var tempMethod = function(url){
			...
			if(flag == 0){
				// 未载入过
				temp[temp.length] = url;
				var caa = '<link href="__PUBLIC__/Css/xxxxxxxxxxxxxxxxxxxxxx" rel="stylesheet" type="text/css" >';
				$('head').append(css);
			}
		};
...
core.plugInit = function() {
	if(arguments.length > 0) {
		...
		var file = PUBLIC_URL + '/Js/plugins/core.' + arguments[0] + '.js';
		core.loadFile(file, back);
	}
};
//与上面方法类似 只不过可以自己写回调函数（不主动执行init）
core.plugFunc = function(plugName,callback){
	var file = PUBLIC_URL+'/Js/plugins/core.'+plugName+'.js';
