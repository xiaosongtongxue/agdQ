...
args['uid']			= uid;
args['cid']			= cid;
args['lastPage']    = lastPage;
args['firstId']		= firstId;
//args['topic_id'] 	= topic_id;

if("undefined" == typeof(core.feed)){	//只init一次
	core.plugFunc('feed',function(){
		core.feed.init(args);
	});
}
/**
...
M.addEventFns({
	...
	insert_face: {	//弹出插入表情框
		click: function(){
			...
			var parentDiv = this.parentModel.parentModel.childModels['faceDiv'][0];
	...
	post_feed:{	//发布普通|图片微博
		click:function(){
			...
			core.feed.post_feed(_this,mini_editor,textarea);/**/
		}
	},
	/*load_more_feed:{
		click:function(){
			var _this = this;
			core.feed.loadMoreFeed(_this);
		}
	},*/
	post_feed_box:{
		click:function(){
			...
			var mini_editor = this.parentModel.parentModel.childModels['mini_editor'][0];
			var textarea = $(mini_editor).find('textarea').get(0);
			core.feed.post_feed(_this,mini_editor,textarea,true);
		}
	},
	post_submission_box:{
		click:function(){
			...
			var mini_editor = this.parentModel.parentModel.childModels['mini_editor'][0];
			var textarea = $(mini_editor).find('textarea').get(0);
			core.feed.post_feed(_this,mini_editor,textarea,true,'submission');
		}
	},
	insert_at:{
		click:function(){
	...
	del_feed:{
		click:function(){
			...
			var delFeed =  function(){
				$.post('/agdQ/index.php/Feed/delFeed',{feed_id:attrs.feed_id},function(msg){
					if(msg.status == 1){
			...
			//ui.confirm(this,'你真的要删除啊?删除后不能恢复哦!',delFeed);
			if(confirm('你真的要删除啊?删除后不能恢复哦!')){
				delFeed();
			}
		}
	},
					...
					if("undefined" != typeof(args.t)) {
						...
						switch ( args.t ){
							...
							case 'feed':
								...
								core.feed.post_feed(postObj,this.parentModel,_this);
								break;
							case 'repostweibo':
...
var getAdminBox = function(feedId)
{
	ui.box.load(U('channel/Manage/getAdminBox')+'&feed_id='+feedId, '推荐到频道');
}