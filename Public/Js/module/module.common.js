...
M.addEventFns({
	...
	share:{//分享操作
		click : function(){
			...
			// 	var sid = attrs.curid;
			// }else{
			var sourse_id = attrs.feed_id;
			//}
			var stable = attrs.table;
	...
	comment:{
		click:function(){	//点击评论的时候
			...
			var comment_container = this.parentModel.parentModel.childModels['feed_comment'][0];
...
//			}else{
				...
				core.comment.init(attrs,comment_container);
				core.comment.display();
//			}
			return false;
		}
	},
	/*$('#feed'+feed_id+' .comment-container').slideToggle('fast');
  	$.post('/agdQ/index.php/Widget/getCommentWidget',{feed_id:feed_id},function(msg){
  		if(msg.status == 0){
	...
	reply_comment:{	//点某条回复
		click:function(){
			var attrs = M.getEventArgs(this);
			var comment_container = this.parentModel.parentModel;
			var docomment = comment_container.childModels['mini_editor'][0].childEvents['do_comment'][0];
			M.setEventArg(docomment,'to_comment_id',attrs.to_comment_id);
			M.setEventArg(docomment,'to_uid',attrs.to_uid);
			M.setEventArg(docomment,'to_comment_uname',attrs.to_comment_uname);
	...
	do_comment:{	//回复操作
		click:function(){
			...
			var after = function(){
				...
				M.setEventArg(_this,'to_comment_id',0);
				M.setEventArg(_this,'to_uid',0);
				//M.setEventArg(_this,'to_comment_uname','');
				/*if(attrs.closeBox == 1){
					ui.box.close();
...
//分享
var share=function(sourse_id,stable,type/*initHTML,curid,curtable,appname,cancomment,is_repost*/){
	var modal = $.scojs_modal({
					title:'分享',
  					keyboard: true,
					remote:'/agdQ/index.php/Widget/share?sourse_id='+sourse_id+'&source_table_name='+stable+'&type='+type
				});
	modal.show();
	core.feed.saveModelObj(modal);
...
*/
	...
	/*$.post('/agdQ/index.php/Widget/share',{sourse_id:sourse_id,source_table_name:stable,type:type},function(msg){
		if(msg.status == 1){
...
var follow = {
	...
	/*doFollow: function(node) {
		var _this = this;
		var args = M.getEventArgs(node);
		var url = node.getAttribute("href") || [U('public/Follow/doFollow'), '&fid=', args.uid].join("");
	...
	/*unFollow: function(node) {
		var _this = this;
		var args = M.getEventArgs(node);
		var url = node.getAttribute( "href" ) || [U('public/Follow/unFollow'), '&fid=', args.uid].join("");
		// 取消关注操作
