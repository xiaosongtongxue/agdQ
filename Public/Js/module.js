...
/**
 * 设置事件节点某个参数（icubit于2013-7-21添加）
 *
 * @param node 事件节点
 * @param key 键
 * @param value 键值
 */
module.setEventArg = function( node, key, value ) {
	var attrs = node.getAttribute( "event-args" );

	var indexOfKey = attrs.indexOf(key);
	//如果找不到key，则不作修改
	if(indexOfKey<0)
		return ;
	var startOfValue = attrs.indexOf('=',indexOfKey)+1;
	var endOfValue = attrs.indexOf('&',startOfValue)-1;
	//如果是最后一个参数
	if(endOfValue<0)
		endOfValue = attrs.length-1;
	var attrs1 = attrs.substring(0,startOfValue);
	var attrs2 = attrs.substring(endOfValue+1);
	attrs = attrs1+value+attrs2;

    node.args = undefined;
    node.setAttribute( "event-args", attrs );
    return module;
};
/**
 * 将uri转换为对象格式
