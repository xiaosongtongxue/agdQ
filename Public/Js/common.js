 ...
 */
//转发、评论、收藏后更新数目,需要优化
var updateCount = function(id,type){
		
		
		if(type == 'comment'){
			var num = 1;
			var comment = $("#feed"+id+" [event-node='comment']:last").find('span').get(0);
			var comment_count = comment.innerHTML;
			if(comment_count != '')
				num = parseInt(comment_count)+1;
			comment.innerHTML=num;
			//source feed的ID号用args属性标示
			//var len = $("#feed_list div[title='source"+id+"']").length;
			$("#feed_list div[args='source"+id+"']").each(function(index, element) {
                comment = $(element).find("a[event-node='comment'] span").get(0);
				comment.innerHTML=num;
            });
			
		}else if(type == "share"){
			var num = 1;
			var share = $("#feed"+id+" [event-node='share']:last").find('span').get(0);
			var share_count = share.innerHTML;
			if(share_count != '')
				num = parseInt(share_count)+1;
			share.innerHTML=num;
			
			$("#feed_list div[args='source"+id+"']").each(function(index, element) {
                share = $(element).find("a[event-node='share'] span").get(0);
				share.innerHTML=num;
            });
		}else{
			return false;
